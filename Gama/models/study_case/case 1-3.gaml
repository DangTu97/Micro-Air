...
//		}
		...
		create vehicle number: 0 {
			name <- flip(CAR_PERCENT) ? (flip(0.1) ? 'CAR' : 'BUS') : 'MOTORBIKE';
...
//			}
			...
			location <- any_location_in(road(0).geom_display);
			do update_polygon;
		...
		loop i from:0 to:6 {
			...
			if (length(var) <= 1 and i != 2) or (i = 2 and length(var) <= 2) {
				create vehicle number: 1 {
					if i = 0 { name <- flip(0.02) ? 'BUS' : 'CAR';}  
					else if i = 1 { name <- flip(0.5) ? 'CAR' : 'MOTORBIKE';}
					else {name <- 'MOTORBIKE';}
