	...
	action get_transfer_geom {
		if (future_node != nil) {
			...
			if (alpha = 0) { // REVIEW IT
				...
				point a1 <- (newpoint + x5)/2;
				transfer_geom <- polygon([newpoint, x5]);
...
//				transfer_geom <- polygon([x2, x8, center, newpoint, x7]);
				...
				transfer_geom <- polyline([x, target_node]);
			}
		} else { // future_node = nil
			float a <- (target_node - start_node).location.x;
			float b <- (target_node - start_node).location.y;    	
...
//		check_turn_right <- (length(vehicle_right) = 0  and (right.location overlaps road_belong.geom_display)) ? true : false;
	...
	action handle_right_corner {
		if (is_transferred = false ) {
			...
			point end <- transfer_geom.points[1];
			point x3 <- (start + end)/2;
			point x2 <- (start + x3)/2;
			point x4 <- (x3 + end)/2;
			
			list<point> closest_list <- [x2,x3,x4] closest_to (location, 1);
			...
			is_transferred <- true;
			start_node <- target_node - (a - end);
			angle <- angle_between(end, end + {10,0}, a);
			transfer_geom <- polyline([start, a]);
		} else {
			do change_road;
		}
	}
	...
	action handle_left_corner {
		if (is_transferred = false) {
			...
			transfer_geom <- polygon([target_node,y]);
			is_transferred <- true;
			// update angle, nodes
...
//			else if (angle_between(start_node, target_node, future_node) < 180) and (location overlaps transfer_geom) {
//				do change_road;
