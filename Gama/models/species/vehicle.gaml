...
species vehicle skills:[moving] {
	...
	road get_road {
		int my_idx <- (road_belong_nodes index_of target_node);
		return (my_idx < length(road_belong_nodes) - 1) ? road_belong : get_next_road(); 
	}
...
//	}
	...
	action decelerate {
//		speed <- max(0.0, speed - deceleration_rate);
		speed <- dead(front_vehicle_conflict) ? max(0.0, speed - deceleration_rate) : 0.9*front_vehicle_conflict.speed;
	}
...
//	}
	...
	action update_bezier_curve {
		...
		float k <- road_belong.is_twoway ? 1 - d/width_of_road_belong 
										 : (angle_between(start_node, target_node, location) <= 180 ? 0.5 - d/(2*width_of_road_belong) : 0.5 + d/(2*width_of_road_belong));
		float dis <- distance_to(target_node, next_node) - compute_distance(location, target_node, orthogonal_target_point);
		road r <- get_road();
		float D <- r.is_twoway ? r.width*(1 - k) : (k <= 0.5 ? r.width*(1-k) : (k - 0.5)*r.width);
		point end_point <- (r.is_twoway = false and k > 0.5) ? get_orthogonal_point(next_node, target_node, compute_distance(location, target_node, orthogonal_target_point), D) 
															 : get_orthogonal_point(target_node, next_node, dis, D);
		float inflection <- compute_inflection(target_node, start_node, next_node);
		float angle_at_corner <- angle_between(target_node, start_node, next_node);
		bool at_right_side <- (angle_at_corner < 180) ? true : false;  
		bezier_curve <- curve(start_point, end_point, inflection, at_right_side, 10);
...
//	}
	...
	action update_bezier_curve_inside_segment(string next_target){
		// left-conflict vehicle or right-conflict vehicle must be nill;
		float h <- dy; 
		point p1 <- location;
		point p2 <- location + direction_unit_vector*h;
		point p_tb <- location + direction_unit_vector*2*h;
		point p3 <- (next_target = "left") ? get_orthogonal_point(p_tb, p1, 2*h, 2*dx): get_orthogonal_point(p1, p_tb, 0.0, 2*dx);
		point p4 <- p3 + direction_unit_vector*h;
		bezier_curve <- curve(p1, p2, p3, p4);
	}
	...
	action pass_vehicle_ahead(string next_target) {
		if (is_passing = false) {
			do update_bezier_curve_inside_segment(next_target);
			is_passing <- true;
			delay_count <- 0;
		}
		do follow_bezier_curve;
...
//	}
	...
	action moving_on_twoway {
		if (is_passing = true) {
			do pass_vehicle_ahead;
		} else {
			if compute_distance(location, target_node, orthogonal_target_point) > 12 {
				if (front_vehicle_conflict = nil) {
					do free_accelerate;
					do moving_straight;
				} else if (left_vehicle_conflict = nil and angle_between(start_node, target_node, left.location) < 180 and compute_distance(left.location, start_node, target_node) > dx) {
					do free_accelerate;
					do pass_vehicle_ahead("left");
				} else if (right_vehicle_conflict = nil and compute_distance(right.location, start_node, target_node) < width_of_road_belong - dx) {
					do free_accelerate;
					do pass_vehicle_ahead("right");
				} else {
					do decelerate;
					do moving_straight;
				}
	...
	action moving_on_oneway {
		if (is_passing = true) {
			...
			if (front_vehicle_conflict = nil) {
				do free_accelerate;
				do moving_straight;
			} else if (left_vehicle_conflict = nil and compute_distance(left.location, start_node, target_node) < width_of_road_belong - dx) {
				do free_accelerate;
				do pass_vehicle_ahead("left");
			} else if (right_vehicle_conflict = nil and compute_distance(right.location, start_node, target_node) < width_of_road_belong - dx) { 	
				do free_accelerate;
...
//		float inflection <- compute_inflection(angle_at_corner, compute_distance(location, start_node, target_node), 0.35, 0.5, width_of_road_belong);
	...
	action handle_conflict_space(traffic_light light_query, float alpha, traffic_light light, traffic_light opposite_light) {
		geometry polygon_check <- front;
//		geometry polygon_check <- (alpha = 180) ? current : front;
